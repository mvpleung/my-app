 ...
 * @Last Modified time: 2018-03-28 17:47:27
 */
<template>
  <div class="staggering-buy">
    <div class="page-header">
      <h3 class="page-header-title">{{ productDetails.parkingName }}</h3>
      <span class="page-header-label">
        {{ productDetails.parkAddress }}{{ distanceDesc }}
      </span>
    </div>
    <div class="page-container">
      ...
      <mt-cell title="错峰日"
        :value="staggeringTime" />
      <mt-cell title="错峰时段"
        :value="productDetails.staggeringTime" />
      <mt-cell title="停放位置"
        :value="productDetails.parkingPosition" />
      <mt-cell title="费用"
      ...
      <coupon-check v-model="checkedCoupon"
        :params="{parkCode:productDetails.parkingCode,productType: '1'}" />
    </div>
    ...
    <div class="page-container page-container--margin">
      <mt-cell class="mint-cell--large">
        <template slot="title">
          <span class="mint-cell-text">错峰停车产品使用须知</span>
          <span class="mint-cell-label">
            1、用户成功购买错峰产品并生效后，可在有效时间内任意进出相应停车场，无需另行缴费
          </span>
          <span class="mint-cell-label">
            2、用户须严格按照错峰停车协议条款执行停车操作
          </span>
          <span class="mint-cell-label">
...
export default {
  ...
  computed: {
     ...
     */
    staggeringTime() {
      if (this.$utils.isEmptyObject(this.productDetails)) return '';
      const chartArray = ['一', '二', '三', '四', '五', '六', '日'];
      let weekArray = (this.productDetails.staggeringWeek || '')
        .split(',')
        .sort((a, b) => Number(a) > Number(b));
      return weekArray.length > 0 ?
  ...
  methods: {
    init() {
      this.productDetails =
        this.$utils.getSessionItem('staggeringPeak', true) || {};
    ...
    paySuccess() {
      ...
      this.$messagebox
        ...
        .then(action => this.$router.replace('/mine/bills?selected=2'));
    }
  },
  components: {
