 ...
 * @Last Modified time: 2018-02-01 13:44:03
 */
<template>
  <div class="amap-wrapper">
    <el-amap-search-box class="search-box" :search-option="searchOption" :on-search-result="onSearchResult"></el-amap-search-box>
    <el-amap class="amap-box" :style="{width: mapStyle.width + 'px', height: mapStyle.height + 'px'}" :vid="'amap-vue'" :plugin="plugin" :center="mapCenter" :zoom="zoom">
      <el-amap-marker v-for="(marker, index) in markers" :key="index" :position="marker.position" :events="marker.events" :content="marker.content"></el-amap-marker>
      <el-amap-info-window v-for="(window, index) in windows" :key="index + 1000" :position="window.position" :visible="window.visible" :content="window.content"></el-amap-info-window>
    </el-amap>
    <div :style="{height: mapStyle.height + 'px', overflow: 'auto'}">
      <mt-cell v-for="(poi, i) in pois" :key="i + 10000" :title="poi.name" :label="poi.address" @click.native="cellClick(i)"></mt-cell>
    </div>
  </div>
</template>
