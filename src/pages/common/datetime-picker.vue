...
<script type="text/babel">
  ...
  export default {
    ...
    created() {
       this.$nextTick(()=>{
            this.$refs.picker.$el.addEventListener('touchmove', event => {
    ...
    methods: {
      ...
      setSlotsByValues(values) {
        ...
        [].forEach && [].forEach.call(this.$refs.picker.$children, child => child.doOnValueChange());
      },
      ...
      confirm() {
        this.visible = false;
        this.$emit('confirm', this.pattern ? this.$utils.formatDateTime(value, this.pattern) : this.currentValue);
      },
