<template>
  <mt-popup v-model="visible" :position="position" class="mint-data">
    <mt-picker
      :slots="slotsData"
      @change="change"
      ...
      <span class="mint-data-action mint-data-cancel" @click="close">{{ cancelText }}</span>
...
<script type="text/babel">
  export default {
    ...
    created(){
        this.$nextTick(()=>{
            this.$refs.dataPicker.$el.addEventListener('touchmove', event => {
      ...
      updateDefault() {
        if(this.default){
          ...
          this.slots.forEach(item => {
              let indexOf = -1;
              item.values && item.values.forEach((value, index) => {
                  if(indexOf === -1 && (value.value === (that.default.value || that.default.val) || value.label === (that.default.label || that.default.val_text))){
                      indexOf = index;
                      defaultValue = value;
                  }
                  if(opt) { //配置可选项
