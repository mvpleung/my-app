<template>
  <div :class="[sizeClass, 'img-inputer-area']">
    <i v-if="showRemove" class="iconfont icon-shanchu1 remove-icon" @click="remove"></i>
    <div class="img-inputer" :class="[themeClass, sizeClass, nhe ? 'nhe': '', showRemove ? 'img-inputer-margin' : '']" ref="box">
      <!--<i class="iconfont img-inputer__icon" v-html="iconUnicode"></i>-->
      <i class="iconfont img-inputer__icon" style="color: white;">+</i>
      <p class="img-inputer__placeholder">{{placeholder}}</p>
      <div class="icon-background"></div>
      <div class="img-inputer__preview-box" v-if="imgSelected || imgShow">
        <img :src="mydataUrl" class="img-inputer__preview-img">
      </div>
...
//      this.$emit('input', this.file);
                ...
                that.resetInput();
				        console.log(that.fileName, '压缩前：', (that.file.size / 1024).toFixed(2), 'KB');
				        console.log(that.fileName, '压缩后：', (rst.fileLen/1024).toFixed(2), 'KB');
				        if(rst.fileLen/1024 > 1024){ //大于 1M
