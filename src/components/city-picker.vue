...
<template>
  <transition name="city-picker-transition">
    <div class="city-picker-container" @click="handleWrapper" v-show="visible">
      <transition v-on:after-enter="afterEnterAnimation" v-on:before-leave="beforeLeaveAnimation" name="citypicker-slide">
          ...
          <div class="con">
            ...
            <div class="tab-con">
              <div class="area" v-show="curTab === 0">
                <span v-for="value in arrays[0]" :key="value.code" @click="handlerClick(value)" :class="getSelected(0, value).select ? 'selected' : ''">
                  {{value.name}}
                </span>
              </div>
              <div class="area" v-show="curTab === 1">
                <span v-for="value in arrays[1]" :key="value.code" @click="handlerClick(value)" :class="getSelected(1, value).select ? 'selected' : ''">
                  {{value.name}}
                </span>
              </div>
              <div class="area" v-show="curTab === 2">
                <span v-for="value in arrays[2]" :key="value.code" @click="handlerClick(value)" :class="getSelected(2, value).select ? 'selected' : ''">
                  {{value.name}}
                </span>
              </div>
            </div>
      ...
      this.axios({
        ...
        if (Array.isArray(resp.data)) {
          if (resp.data.length > 0) {
            ...
            this.arrays[item.code.match(/^\d{2}0000$/) ? 0 : item.parentCode.match(/^\d{2}0000$/) ? 1 : 2] = resp.data;
          } else {
