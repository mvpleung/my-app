module.exports = (function () {
  ...
  function isVerifyConfig(expression) {
    return expression.indexOf('|') === -1 && expression.indexOf('\'') === -1 && is('String', expression);
  }
  ...
  function defineErrors(vm, el, field, attr) {
    ...
    defineDataPro(vm, 'errors', { [field]: errors && Array.isArray(errors) ? errors : [] }, true);
  }
  ...
  function defineAttr(vm, el, binding, callback) {
    ...
    if (binding.name === 'remind') { //有定义 v-remind，缓存定义 v-remind 的字段，错误提示使用 remind 提示代替默认提示
      defineDataPro(vm, 'remind', { [binding.field]: el }, true);
    } else if (binding.name === 'verify') { //定义了 v-verify 的字段，提取自定义属性中的 blur 字段，用于失去焦点校验
      defineDataPro(vm, 'blur', { [binding.field]: verifyAttr.blur === true || verifyAttr.blur === 'true' }, true);
    }
    //存储原始节点
   ...
   * @param {Object} options 配置项
   */
  function batchMsg(vm, options) { //暂时屏蔽，批量校验仅支持行内错误提示
    if (options && options.multiple && options.msgbox && vm.$verify.$errorArray && vm.$verify.$errorArray.length > 0) {
  ...
  function scrollToVerifyEl(vm, field, offsetTop) {
    ...
    //不在可见区域内时，滚动
    if (el && !(el.offsetTop + el.offsetHeight / 3 >= window.pageYOffset && el.offsetTop + el.offsetHeight / 3 < window.pageYOffset + window.outerHeight)) {
  ...
  function replaceError(vm, field, rule, index) {
    if (rule) {
      let errors = (vm.$verify.protoQueue.errors ? vm.$verify.protoQueue.errors[field] : []) || [];
  ...
  function convertCustomError(vm, field, expression) {
    ...
    verifyExpression = replaceError(vm, field, Object.assign(verifyExpression, verifyExpression, defaultRe));
  ...
  function ignore(vm, field) {
    ...
    if (original && original.hasAttribute('data-verify')) {
      ...
      if (verifyAttr && (verifyAttr.ignore === true || verifyAttr.ignore === 'true')) {
        return true;
      }
    }
    return false;
  ...
  function isMsgBox(vm, options, field, validOnly) {
    return options.force === true || !options.multiple && !validOnly && fieldOption(options).msgbox !== false && options.msgbox !== false && !(vm.$verify.protoQueue.remind && vm.$verify.protoQueue.remind.hasOwnProperty(field));
  }
  ...
  function isSupportBlur(vm, field) {
    return vm.$verify.protoQueue.blur && vm.$verify.protoQueue.blur[field] === true && !ignore(vm, field);
  }
