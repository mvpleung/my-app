 ...
 * @Last Modified time: 2018-02-11 09:48:14
 */
...
let rules = {
  ...
  fullname: {
    test: function (value) {
      return value ? /^[\u2E80-\u9FFF]+[·|\s|\u2E80-\u9FFF]*[\u2E80-\u9FFF]+$/.test(value) || /^[a-zA-Z]+[•|\s|a-zA-Z]*[a-zA-Z|•]+$/.test(value) : false;
    },
    message: '请正确输入姓名'
  },
  number: { //判断字符串是否为数字
  ...
  positiveNum: { //判断是否为正整数+正浮点数
    test: /^(([1-9][0-9]*)|(([0]\.\d{1,}|[1-9][0-9]*\.\d{1,})))$/,
    message: '请输入正整数或正浮点数'
  },
  address: { //是否是合法地址（必须包含汉字，不能连续5个相同字符，不少于16个字节）
    test: function (value) {
      ...
      return /[\u4E00-\u9FA5]/g.test(value) && !/(\S+)\1{7}/ig.test(value) && getBytesCount(value) >= 16;
    },
    message: '详细地址必须包含汉字，不能连续5个相同字符，最少8个汉字，长度不少于16个字节'
  },
  qq: { //是否是QQ号
    test: /[1-9]([0-9]{4,11})/,
    message: '请正确输入QQ号码'
  ...
  password: { //是否符合密码规则校验，长度6-20位 数字+字母
  ...
  minLength: { //最小长度
    test: function (value, rule) { //rule 定义的规则的值
  ...
  max: { //最大值
    test: function (value, rule) {
      return value && rule && Number(value) <= Number(rule || Number.POSITIVE_INFINITY);
    },
